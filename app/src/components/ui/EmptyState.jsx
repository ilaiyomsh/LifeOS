...
export default function EmptyState({ icon: Icon, title, description, className }) {
  return (
    <div className={cn('flex flex-col items-center justify-center py-12 px-6 text-center', className)}>
      ...
      <h3 className="text-sm font-semibold text-slate-600 dark:text-gray-300 mb-1">{title}</h3>
      {description && <p className="text-xs text-slate-400 dark:text-gray-500 max-w-[240px]">{description}</p>}
    </div>
  );
}
